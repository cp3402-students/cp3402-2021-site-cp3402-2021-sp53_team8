		...
		public function dispatch($action) {
			...
			elseif ( !empty($action) ) {
				switch($action) {
					...
					// Delete content
					case 'empty':
						if ( defined('WP_CLI') || check_admin_referer( 'empty', 'fgj2wp_nonce' ) ) { // Security check
					...
					case 'modify_links':
						if ( defined('WP_CLI') || check_admin_referer( 'modify_links', 'fgj2wp_nonce' ) ) { // Security check
							$this->modify_links();
...
SQL;
		...
		private function validate_form_info() {
			...
			return array(
				...
				'archived_categories'	=> filter_input(INPUT_POST, 'archived_categories', FILTER_SANITIZE_STRING),
				'unpublished_categories'=> filter_input(INPUT_POST, 'unpublished_categories', FILTER_SANITIZE_STRING),
				...
				if ( !empty($featured_image) ) {
					...
					if ( !empty($result['media']) ) {
						foreach ( $result['media'] as $featured_image_id ) {
							if ( $this->plugin_options['remove_first_image'] || $this->plugin_options['only_featured_image'] ) {
								$this->media_count++;
							}
							break;
						}
